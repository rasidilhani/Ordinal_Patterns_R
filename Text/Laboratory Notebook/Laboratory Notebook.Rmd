...
abstract: |
  This research focuses on identifying perturbations in machines using time series data, a process where detecting chaotic patterns poses significant challenges. Bandt and Pompe introduced a novel approach to analyze time series through ordinal patterns, which facilitates the extraction of qualitative information from nonlinear data efficiently and automatically. Their concept of permutation entropy provides a measure of the complexity inherent in a system, leveraging the ordinal patterns that capture variations within a time series.However, permutation entropy addresses only one aspect of a time series’ ordinal structure, which limits its scope. A critical challenge in pattern classification systems lies in effective feature selection. This study investigates methods to provide classification systems with the most defining features for ordinal multi-class classification. By integrating cluster and variability analyses, an innovative and efficient feature selection scheme is proposed. Additionally, the research demonstrates how to process real-valued datasets to extract all relevant ordinal information, advancing the ability to analyze and classify complex time series data comprehensively.  Experimental results show that our feature selection scheme sifts successfully a compact subset of characterizing features for classification problems with multiple classes.
...
Furthermore we explore the advantages of ordinal pattern analysis in detecting changes in system dynamics associated with faults. By identifying alterations in the frequency and recurrence of specific temporal patterns, this technique can reveal subtle indicators of developing faults. Furthermore, unlike some traditional methods limited to linear systems, ordinal pattern analysis can effectively detect faults in complex, non-linear systems, broadening its applicability. 

Feature selection and modern classification challenges have grown increasingly intricate due to the exponential rise in data generation [@LIN201394]. Current databases can handle diverse data types, and the multi-dimensional nature of modern datasets often results in individual records encompassing numerous attributes or features. Moreover, contemporary classification tasks frequently involve choosing between multiple possible classes, further complicating decision-making processes.

The surge in data complexity and the increasing number of class options have negatively impacted classification performance and accuracy. To address these challenges, strategies like selective sampling and feature reduction are often employed. Selective sampling focuses on excluding outlier or noisy data from analysis and computation. However, distinguishing between standard and atypical instances can sometimes be ambiguous. Additionally, selective sampling may face issues such as insufficient data or overly large sample sizes.

Feature reduction serves as an alternative approach, especially in cases involving datasets with hundreds or thousands of attributes. High-dimensional datasets often emerge from combining multiple data sources or merging related datasets. Feature reduction identifies and retains a smaller subset of critical classification factors, simplifying the problem without compromising essential information. While various feature reduction methods exist, there remains a gap in optimizing both necessary and sufficient factors simultaneously for enhanced classification efficiency.
...
Permutation entropy offers a reliable measure of complexity that is less susceptible to noise and does not require linear or Gaussian data properties by taking into account the relative ordering of values within a time series rather than their numerical magnitude. Our goal is to use this idea to make ordinal pattern analysis a practical and beneficial tool for diagnosing bearing faults, advancing predictive maintenance technologies, and encouraging more dependable and efficient operation of rotating machinery.

The rest of this article is organized in the following order: 
Section 2 discusses the literature review on bearing fault analysis and cluster-based classification applications for ordinal patterns. Section 3 and 4 introduces the concept of permutation entropy and the ordinal pattern theory concept of complexity-entropy curves. Section 5 examines several simulated time series in the context of noisy periodic signals to test the performance of the proposed method. Section 6 demonstrates the empirical analysis of bearing fault analysis and geomagnetic dynamics. Finally, Section 7 concludes the paper with some conclusions and future research.
