...
The paper explores an on-line health status estimation approach for rolling bearings based on vibration signals, focusing on real-time monitoring and analysis to assess bearing condition; the study demonstrates that this approach effectively estimates health status by continuously evaluating vibration data to detect and predict potential faults [@WOS:000452922000015]. The paper presents an automatic bearing fault classification method using deep learning techniques, which leverages neural networks to analyze and classify fault patterns from vibration signals, resulting in improved accuracy and efficiency in detecting and diagnosing bearing faults [@WOS:000453413600001]. The paper introduces an early fault diagnosis method for bearings using Empirical Wavelet Transform (EWT) combined with energy entropy, which enhances the detection of incipient faults by analyzing vibration signals with refined frequency decomposition and measuring signal energy distribution, resulting in improved early fault identification accuracy [@WOS:000452819600235]. [@WOS:000450745100001, @WOS:000449334500118] presents a method for rolling element bearing fault detection that uses segmentation of vibration signals combined with wavelet analysis and deep neural networks; this approach enhances fault detection by dividing the signal into meaningful segments for improved accuracy, leveraging wavelet transforms for feature extraction and deep neural networks for clustering and classification, resulting in more precise identification and categorization of bearing faults.

[@WOS:000440977000032] paper introduces a method for bearing fault feature extraction using autoregressive coefficients, Linear Discriminant Analysis (LDA), and Support Vector Machines (SVM) under variable operating conditions, combining these techniques to enhance fault detection by accurately extracting and classifying fault features despite changing operational environments. A novel fault diagnosis method for rolling element bearings that combines Noise-Assisted Multivariate Empirical Mode Decomposition (MEMD) with a Functional Neural Fuzzy Network, leveraging noise-assisted MEMD for effective feature extraction and the neural fuzzy network for precise fault classification, resulting in improved diagnostic accuracy and robustness[@WOS:000434717400001]. [@WOS:000426284100001] introduces a rolling bearing fault diagnosis method that employs Modified Local Fisher Discriminant Analysis (LFDA) and Empirical Mode Decomposition (EMD) combined with sensitive feature selection, aiming to enhance fault detection by extracting and analyzing critical features from vibration signals with improved accuracy and discrimination. The paper [@WOS:000539546400083] presents an early fault diagnosis method for rolling bearings that integrates Empirical Wavelet Transform (EWT) with Spectral Kurtosis, focusing on detecting incipient faults by utilizing EWT for refined frequency analysis and Spectral Kurtosis to highlight abnormal signal features, leading to improved early fault identification and diagnostic accuracy. The paper [@WOS:000426986200020] introduces an improved similarity-based modeling approach for classifying rotating machine failures, enhancing the accuracy and effectiveness of fault classification by leveraging advanced similarity measures to better match failure patterns and improve diagnostic precision; results demonstrate that this method significantly outperforms traditional classification techniques, achieving higher accuracy and reliability in failure detection. The article [@HAKIM:2023101945] provides a comprehensive review of rolling bearing fault diagnosis methods that utilize deep learning and transfer learning, offering a detailed taxonomy, overview, and application of these techniques. It also addresses open challenges, weaknesses, and provides recommendations for future research. The review highlights that deep learning and transfer learning approaches have shown significant improvements in fault diagnosis accuracy and robustness, but also notes ongoing issues such as the need for more extensive and diverse datasets and the challenge of model generalization across different operating conditions. The review suggests that addressing these challenges can further enhance the effectiveness and applicability of these advanced diagnostic methods.  
