...
As we discussed earlier, real-valued time series $\bm{x}=\{x_1,x_2,...,x_{n+D-1}\}$  transform into the series symbols $\bm{{\pi}}=({\pi}_1, {\pi}_2,\dots, {\pi}_n)$ from sub-sequences of embedding dimension $D$, where we considered $D!=k$. Due to the overlapping of time windows, the ordinal patterns which we calculate are dependent. For $i=1,2,...k$, let $p_i$ be the probability of observing the state $\pi_i$, denote the vector probabilities, $\bm{p}={\left\{p_1,p_2,\dots,p_k\right\}}$ and express as $\bm{D_p}=\text{Diag}(p_1,p_2, \dots, p_k)$ the diagonal matrix. 
...
\begin{equation}
  H(\widehat{\bm{p}})\pm \dfrac{Z_{\alpha/2}\sigma_{\bm{p}}}{\sqrt{n}},
...
\begin{equation}
	...
	\label{eq:CI}
\end{equation} 
where $\textbf{Semi Length}=\dfrac{Z_{\alpha/2}\sigma_{\bm{p}}}{\sqrt{n}}$
