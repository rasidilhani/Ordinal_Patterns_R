...
This proposal focuses on the independent analysis of permutation entropy and ordinal patterns in time series analysis. It involves calculating pattern histograms, entropy, complexity, and confidence intervals to better understand the statistical properties of these tools. 
...
These methods rely on the rank $R_t$ of the observations $x_t$ rather than their actual values, making them robust and applicable to a wide range of data sets. 
Since non-parametric tests do not assume a normal distribution, they are highly reliable. 
For example, the Kruskal-Wallis $H$ test 
%%% ACF Do you use the KW test again? You have defined H as the notation for its statistics, while H becomes entropy
and the Wilcoxon test are effective tools for comparing two or more population probability distributions from independent random samples. However, these techniques are not always suitable for time series data, which often require specialized methods tailored to their unique characteristics.
...
Assume we use patterns of length $D=3$ with a time lag $\tau=1$. 
...
Chapter~\ref{C:lit} will review the literature on ordinal pattern analysis, while Chapter~\ref{C:aim} will expand on this concept by exploring the characterization of time series. It will also cover the computation of two key descriptors entropy and complexity from the resulting histograms. Additionally, Chapter~\ref{C:futw} will outline the main ideas and objectives of this research project.      
