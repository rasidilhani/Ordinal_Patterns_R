...
# Function to generate all possible permutations of a given length
generate_permutations <- function(n) {
  permute <- function(x) {
    ...
    for (i in 1:length(x)) {
      ...
      out <- c(out, paste(x[i], sub_perms, sep = ""))
    }
    return(out)
  }
  return(permute(1:n))
...
plot_frequencies <- function(freq_df, title_txt, emb_dim) {
  patterns <- generate_permutations(emb_dim)
  freq_df <- merge(data.frame(Pattern = patterns), freq_df, by = "Pattern", all.x = TRUE)
  freq_df[is.na(freq_df)] <- 0
  ggplot(freq_df, aes(x = factor(Pattern, levels = patterns), y = Probability)) +
...
# Define the range of embedding dimensions
embedding_dimensions <- 3:7   

# Initialize a list to store results
complexity_results <- list()

# Calculate and plot results for each embedding dimension
for (emb_dim in embedding_dimensions) {
  ...
  # Plot frequencies for a specific embedding dimension
  print(plot_frequencies(probabilities_df, paste("Embedding Dimension", emb_dim), emb_dim))
}

# Combine all results into a single data frame
complexity_df <- do.call(rbind, complexity_results)

# Plot Shannon Entropy vs JS Entropy
ggplot(complexity_df, aes(x = PermutationEntropy, y = JensenShannonEntropy, color = as.factor(Emb_Dim), group = Emb_Dim)) +
  geom_point(size = 4) +
  geom_line(linetype = "dashed") +
  labs(x = "Permutation Entropy", y = "Jensen-Shannon Entropy", color = "Embedding Dimension") +
  theme_minimal() +
  ggtitle("Shannon Entropy vs Jensen-Shannon Entropy")
