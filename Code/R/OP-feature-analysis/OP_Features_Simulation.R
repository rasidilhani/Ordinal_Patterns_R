...
D <- 3  # Embedding dimension (should match simulation)

# --- Define file paths ---
data_path   <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Data/OP_Features n1000_n5000/D3 results_tau_1/HC_Results_all_Models_extended_entropies_D3.xlsx"
output_path <- data_path  # same file

#C:\Users\UserA1\Documents\GitHub\Ordinal_Patterns_R\Data\OP_Features n1000_n5000\D3 results_tau_1\HC_Results_all_Models_extended_entropies_D3
...
# --- Storage lists for n = 1000 and n = 5000 ---
results_1000  <- list()
results_5000 <- list()
...
for (sheet in all_sheets) {
  ...
  # Extract model name and n value from sheet name, e.g. "AR1_M1_n1000"
...
data_path   <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Data/OP_Features n1000_n5000/D3 results_tau_1/HC_Results_all_Models_extended_entropies_D3.xlsx"
base_plot_dir <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Plots/ARMA plots/OP_Features Plots n1000_n5000"
...
#======================================================================
#This script generates scatter plots of complete entropy vs. complexity curved for various ARMA models,
#using extended entropy measures (Shannon, Rényi, Tsallis, Fisher).
#======================================================================
# --- Required Packages ---
library(readxl)
...
for(case_name in names(cases)) {
  ...
  for(n_val in sample_sizes) {
    ...
    for(ent_name in names(entropy_configs)) {
      ...
      # --- Use FULL Linf/Lsup boundaries (Shannon only) ---
      if (cfg$add_bounds) {
        Linf_full <- LinfLsup_subset  # Use complete theoretical curve
      } else {
        Linf_full <- NULL
      }
      ...
      # --- Scatter Plot for this entropy type ---
      p_scatter <- ggplot() +
        # Full theoretical curve as background (Shannon only)
        {
          if (!is.null(Linf_full)) {
            list(
              geom_line(
                data = subset(Linf_full, Side == "Lower"),
                aes(H, C),
                color = "gray60", linewidth = 1.0, alpha = 0.8
              ),
              geom_line(
                data = subset(Linf_full, Side == "Upper"),
                aes(H, C),
                color = "gray60", linewidth = 1.0, alpha = 0.8
              )
            )
        ...
        scale_shape_manual(values = model_shapes) +
        labs(
          title = paste0(ent_name, " HC Scatter",
                         if (cfg$add_bounds) " with Boundaries " else " ",
                         "(", case_name, ", n=", n_val, ")"),
          x = cfg$xlab,
          y = cfg$ylab
      ...
      # --- Save Plot ---
      file_name <- paste0("Full Boundary Scatter_", ent_name, "_", case_name, "_n", n_val, ".pdf")
      ggsave(file.path(case_plot_dir, file_name),
             p_scatter, width = 8, height = 6)
...
message("🎉 All HC full boundary scatter plots completed (LinfLsup only for Shannon)!")
