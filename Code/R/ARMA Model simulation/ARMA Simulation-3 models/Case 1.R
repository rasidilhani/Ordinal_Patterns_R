...
plot_dir <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Plots/ARMA plots/Case 1 results"
if (!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE)

output_csv <- file.path(plot_dir, "Entropy_Complexity_AllModels_case1.csv")
...
summary_csv <- file.path(plot_dir, "Entropy_Complexity_Model_Summary_case1.csv")
...
output_dir <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Plots/ARMA plots/Case 1 results"
...
for (etype in unique(combined_df$Entropy_Type)) {
  ...
  plots_list[[etype]] <- p
  ggsave(filename = file.path(output_dir, paste0(etype, "_Plane_case1.pdf")),
...
# Save combined plot
ggsave(filename = file.path(output_dir, "All_Entropy_Comparison_case1.pdf"),
...
plot_dir <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Plots/ARMA plots/Case 1 results/Emblematic results"
if (!dir.exists(plot_dir)) dir.create(plot_dir, recursive = TRUE)
...
# Also save CSV
output_csv <- file.path(plot_dir, "Emblematic_All_Raw_case1.csv")
...
model_shapes <- c(
  "AR1_M1" = 16, "AR1_M2" = 1, "AR2_M1" = 17,  
  "MA1_M1" = 15, "MA1_M2" = 0, "MA2_M1" = 18, 
  "ARMA11_M1" = 5, "ARMA11_M2" = 2, "ARMA22_M1" = 8
)
...
for (etype in unique_entropies) {
  ...
  ggsave(
    filename = file.path(plot_dir, paste0("Median_HC_", etype, "_Entropy_case1.pdf")),
...
p_grid <- grid.arrange(
  ...
  ncol = 1,
  heights = c(10, 1),
  top = "Entropy–Complexity Median Comparison:Case 1"
)
...
ggsave(
  filename = file.path(plot_dir, "Median_Comparison_4in1_case1.pdf"),
...
plot_dir <- "C:/Users/UserA1/Documents/GitHub/Ordinal_Patterns_R/Plots/ARMA plots/Case 1 results/Time series and median points"

# Read HC median points (sheet 1)
hc_med <- read_excel(data_path, sheet = 1)

# Read full time series data (sheet 2)
ts_data <- read_excel(data_path, sheet = 2)

# Models to plot
models_to_plot <- c("AR1_M1", "AR1_M2", "AR2_M1", 
...
ggsave(filename = file.path(plot_dir, "HC_plane_median_points_case1.pdf"), plot = p_hc, width = 8, height = 5)
...
combined <- grid.arrange(
  ...
  widths = widths,
  heights = heights,
  top = textGrob("Median points in the H-C plane and associated Time Series (Case 1)",
                 gp = gpar(fontsize=16, fontface="bold"))
)

# Save to file
ggsave(file.path(plot_dir, "Combined_HC_TS_grid_10plots_case1.pdf"),
