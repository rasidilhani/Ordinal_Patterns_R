...
n <- 5000
sample_data <- rnorm(n)
save(sample_data, file = "sample_data_Tsallis.RData")  
...
for (i in seq_len(nrow(param_grid))) {
  ...
  TEntropy <- HTsallis(ProbARMA, beta = 1.5)
...
save(results, file = "arma_results_Tsallis.RData")  # Save results for flexible plotting

# 4. Plot results (can reload the results for future plots without re-running simulation)
load("arma_results_Tsallis.RData") # Use if you've already run the simulation and saved results

#data("LinfLsup")  

ggplot(results, aes(x = TEntropy, y = Complexity, color = ParameterSet)) +
  geom_point(size = 2) +
  #geom_errorbarh(aes(xmin = TEntropy - semiLengthPD, xmax = TEntropy + semiLengthPD), height = 0.02) +
  ...
  labs(
    title = paste("Tsallis Entropy and Complexity for ARMA Time Series (D =", D, ", n =", n, ")"),
