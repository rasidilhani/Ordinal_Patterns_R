...
create_ma2_plot <- function(output_data, title) {
...
for(i in 1:length(ma2_models)) {
  ...
  individual_plots[[i]] <- create_ma2_plot(
    model_data,
    paste(model_name, "(MA =", paste(ma_coef, collapse = ", "), ")")
  )
  all_ma2_data <- rbind(all_ma2_data, model_data)
}
...
dummy_bounds <- get_plot_bounds(all_ma2_data)

dummy_plot <- ggplot() +
  geom_point(data = all_ma2_data,
             aes(x = Entropy, y = Complexity, col = n), alpha = 0.7, size = 1.5) +
  theme_pander() +
...
combined_plot <- arrangeGrob(
  ...
  top = "MA(2) Models - Ordinal Pattern Analysis"
)
grid.arrange(combined_plot, shared_legend, nrow = 2, heights = c(10, 1))

ggsave(
  "MA2_combined_analysis.pdf",
