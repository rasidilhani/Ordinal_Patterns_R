...
generate_ar1_data <- function(ar_coef, model_name, n, r_iterations = R) {
  Output <- NULL
...
create_ar1_plot <- function(output_data, title) {
...
# Data and individual panels
all_ar1_data <- data.frame()
individual_plots <- list()
for(i in 1:length(ar1_models)) {
  model_name <- names(ar1_models)[i]
  ar_coef <- ar1_models[[i]]$ar
  model_data <- data.frame()
  for(n in N) {
    temp_data <- generate_ar1_data(ar_coef, model_name, n)
    model_data <- rbind(model_data, temp_data)
  }
  individual_plots[[i]] <- create_ar1_plot(
    model_data,
    paste(model_name, "(AR =", ar_coef, ")")
  )
  all_ar1_data <- rbind(all_ar1_data, model_data)
}
...
dummy_bounds <- get_plot_bounds(all_ar1_data)

dummy_plot <- ggplot() +
  geom_point(data = all_ar1_data,
             aes(x = Entropy, y = Complexity, col = n), alpha = 0.7, size = 1.5) +
  theme_pander() +
...
combined_plot <- arrangeGrob(
  ...
  top = "AR(1) Models - Ordinal Pattern Analysis"
)
grid.arrange(combined_plot, shared_legend, nrow = 2, heights = c(10, 1))

ggsave(
  "AR1_combined_analysis.pdf",
...
all_ar1_data <- data.frame()
for (i in seq_along(ar1_models)) {
  model_name <- names(ar1_models)[i]
  ar_coef <- ar1_models[[i]]$ar
  temp_data <- generate_ar1_data(ar_coef, model_name, N)
  all_ar1_data <- rbind(all_ar1_data, temp_data)
}
write.csv(all_ar1_data, "AR1_All_Entropy_Complexity_Final.csv", row.names = FALSE)

cat("✅ AR(1) entropy–complexity and complexities saved as 'AR1_All_Entropy_Complexity_Final.csv'\n")
