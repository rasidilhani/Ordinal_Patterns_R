...
for (i in seq_len(nrow(param_grid))) {
  ...
  label <- param_grid$label[i]
    ...
    print(label)
    ts_data <- arima.sim(model = list(ar1 = ar1, ar2 = ar2, ma1 = ma1), n = n)
...
save(results, file = "arma(12)_results.RData")  # Save results for flexible plotting
write_xlsx(results, path = "arma(12)_grid_results.xlsx")

# 4. Plot results 
load("arma(12)_results.RData") # Use if you've already run the simulation and saved results
