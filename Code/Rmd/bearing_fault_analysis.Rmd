...
de_time_data <- df[df$Motor_load == MD, ]$DE_time
fe_time_data <- df[df$Motor_load == MD, ]$FE_time
ba_time_data <- df[df$Motor_load == MD, ]$BA_time
...
HCPoints <- data.frame(H=ShannonEntropies,
                       ...
                       Series=as.factor(c("DE time", "FE time")))
...
pdf_file_name <- file.path(directory_path_complexity_plane, paste(sprintf("confidence_interval_%s_%s", D, 0), ".pdf"))
